...
def test_the_reader_names_no_default_header():
    """`from_header()` must not be callable bare: a default header name would make
    trusting one the thing that happens when nobody decides.
    """
...
def test_a_source_with_no_vocabulary_refuses_to_start(cfg):
    """The converse, and the more dangerous one: a deployment somebody believes is
...
def test_the_refusal_names_both_halves(policied):
    """A message that said only 'misconfigured' would leave the reader to work out which
