...
**The first event is pulled before the response starts.** `astream` does `_prepare`
as its first statement and yields nothing before it, so every refusal a turn can make
-- unknown session, busy, quota, a capability not granted, a definition that will not
parse -- is available before any event exists. Pulling it here means those become
status codes. Handing the generator straight to `StreamingResponse` would put 200 on
the wire first and bury every refusal in the stream, where a client has to parse the
body to learn it failed.
"""
