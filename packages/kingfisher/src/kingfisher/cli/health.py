...
def _packs() -> Iterator[Check]:
    """Whether anything supplies definitions.

    A warning rather than a failure: kingfisher runs perfectly well against a
    workspace somebody filled by hand, and an installation with no pack is a
    choice. It is worth saying because an empty workspace and a missing pack
    look identical from the outside.
    """
    packs = installed_packs()
    if packs:
        yield Check("asset packs", "ok", ", ".join(pack.name for pack in packs))
    else:
        yield Check(
            "asset packs",
            "warn",
            "none installed, so `kingfisher seed` would copy nothing",
            "pip install kingfisher-assets, or write definitions yourself",
        )
