...
def test_a_healthy_workspace_passes_everything(cfg):
    ...
    assert {check.name for check in checks} >= {
        "catalogue", "aliases", "asset packs", "tools", "subagents", "skills", "shell"
    }
...
def test_no_asset_pack_warns_rather_than_failing(cfg, monkeypatch):
    """Kingfisher runs perfectly well against a workspace somebody filled by
    hand. It is worth saying because an empty workspace and a missing pack look
    ...
    # patching `seeding.installed_packs` would change nothing.
    monkeypatch.setattr("kingfisher.cli.health.installed_packs", tuple)

    checks = {check.name: check for check in examine(cfg)}

    assert checks["asset packs"].verdict == "warn"
    assert checks["asset packs"].remedy
