...
A tool is the one of the four that is code rather than a document, so writing
one has a page of its own: [`tools.md`](tools.md). This page says what a
definition may say *about* a tool — which it grants, and which file's when two
...
Python you write, imported from the workspace's `tools/` directory and added to
the built-in set. A module declares `TOOLS`, the list it contributes, and a tool
is a LangChain tool — from `@tool`, from a `BaseTool` subclass you instantiate,
or a plain function.

Writing one is [`guides/tools.md`](tools.md): the three shapes, what a tool may
return, folders and packages, and every refusal the loader has. What a
*definition* says about one is here.

### Two tools with the same name

Two files may each define a `fetch` and both load, which
[`guides/tools.md`](tools.md#two-files-may-define-one-name) covers from the
loader's side. What a definition does about it is here.
