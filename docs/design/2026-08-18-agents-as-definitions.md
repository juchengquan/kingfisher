...
| A23 | **Two agents of one name are refused**, where two subagents of one name are kept and qualified. | A request names exactly one agent, so there is no roster for a reference to disambiguate within: two files claiming `assistant` means a request for `assistant` gets whichever the walk reached last, and nothing anywhere says which. A subagent is selected *from* a roster, so `team/surveyor.yaml::surveyor` has both somewhere to be written and something to pick out — and refusing the pair there stopped deployments over a clash no single agent had asked for. Recorded after the fact: it was settled in the implementation and lived only in a comment in `catalogue/agents.py`, which A9 makes look like an oversight — that entry records a *smaller* asymmetry between the two formats (`subagents: ["*"]` legal in one, refused in the other) while this larger one was written down nowhere a reader would look. |
