...
**Status:** implemented, E1-E22, in three commits.
**Date:** 2026-08-19
...
| E14 | **The README's first example passes the helper**, not a literal path — *changed during implementation.* | It read `seed(paths_from_env())`, which E10 makes a type error — the project's front page taught something that would not compile. The design said a literal `./examples`, and building it showed that to be the same fault E20 exists to prevent: a path inside this repository is false for a reader who installed the package, and the README's reader is not necessarily standing in a checkout. So it shows `definitions_source(paths)`, which is true for everyone and demonstrates the actual mechanism, with a line naming `KINGFISHER_ASSETS` and `--from` underneath. The README's *"What ships separately"* section inverts: it exists to explain why the definitions are not separate, and now has to say that nothing ships but the library. `docs/formats.md` gets the same treatment — the entry-point history stays, its tense does not. |
