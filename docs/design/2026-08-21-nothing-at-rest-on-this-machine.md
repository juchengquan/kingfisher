...
| N16 | **built and asserted** | The saver is built per turn -- `build_session_checkpointer(session_dir)` at `service.py:1249` -- and released when the turn ends, so nothing carries a turn's graph state forward. `test_a_turns_working_state_does_not_reach_the_next_one` drives it: a scripted model writes a plan, the turn boundary is crossed, and the plan is gone while the conversation is not. It was traced but unasserted until 2026-09-01, which was the gap -- a deployment injecting a persistent `threads` factory takes this back, and nothing else would say so. |
