...
| V4 | **The base keeps a small command that says how to install the service.** Ten lines, no imports of the service. | The repo already argued for this — "a script that exists and says 'install the extra' is a better answer than one that is silently absent" — and does not currently deliver it: on a plain base install `kingfisher-server` dies with `ImportError: No module named 'fastapi'`, because the import chain reaches fastapi long before the code that would print the message. The friendly message only appears when fastapi is present and uvicorn is not. This finally keeps the promise. |
