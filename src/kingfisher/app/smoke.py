...
from kingfisher.domain.workspace import writable_data
...
#: Seeded into the workspace rather than shipped in the package: skills are
#: domain content, and a general agent's package should carry none.
SAMPLE_SKILL = """---
name: tabular-qa
...
def seed_sample_skill(workspace: Path) -> bool:
    """Write the sample skill if it is not already there. Returns True if written."""
    target = Path(workspace) / "skills" / SKILL_NAME / "SKILL.md"
    if target.exists() and target.read_text(encoding="utf-8") == SAMPLE_SKILL:
        return False
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(SAMPLE_SKILL, encoding="utf-8")
    return True
