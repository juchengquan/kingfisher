...
by every import of every kind. One reaching `kingfisher.kinds.subagents.harness`
or `kingfisher.kinds.skills.backend` puts deepagents in front of all of them:
`kingfisher.kinds.subagents` is 6ms and 65 modules today, and 1,420ms and 3,164
behind such a line -- so `kingfisher seed` would import three provider SDKs to
read a directory. Each module is imported by name instead, which is what
`agents/__init__.py` already says about its own.
